...
+(IMToast*)showThenHidden:(NSString*)content withView:(UIView*)view afterDelay:(NSTimeInterval)delay
{
    ...
    if (contentW > 20) {
        ...
        if (array != nil && [array count] > 0) {
            ...
            [toast showThenHidden:CGRectMake((view.frame.size.width-contentW-IMTOAST_CONTENTINTERVAL_RIGHT-IMTOAST_CONTENTINTERVAL_LEFT)/2, 0,contentW+IMTOAST_CONTENTINTERVAL_RIGHT+IMTOAST_CONTENTINTERVAL_LEFT , ([mArray count]-1)*IMTOAST_CONTENTINTERVAL_SPROW+[mArray count]*IMTOAST_CONTENTINTERVAL_HEIGHTROW+IMTOAST_CONTENTINTERVAL_TOP+IMTOAST_CONTENTINTERVAL_BOTTOM) withWithContentArray:mArray afterDelay:delay];
        }
    }
